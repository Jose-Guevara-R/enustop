...
export const ShortcutsHelpModal: React.FC<ShortcutsHelpModalProps> = ({ isOpen, onClose, darkMode }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/75 backdrop-blur-md p-4">
      <div className={`relative w-full max-w-md rounded-3xl p-6 md:p-7 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.6)] border ${
        ...
        {/* Rules */}
        <div className="rounded-2xl bg-gradient-to-br from-amber-50/80 to-orange-50/80 dark:from-amber-950/40 dark:to-orange-950/30 border border-amber-200 dark:border-amber-800/50 p-4 text-xs space-y-2 mb-4">
          <div className="font-display font-black flex items-center gap-1.5 text-amber-800 dark:text-amber-300 text-sm">
            <ShieldCheck className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
            <span>Reglas & Anti-Trampas</span>
          </div>
          <p className="text-[11px] leading-relaxed text-amber-950 dark:text-amber-200">
            • <strong>Orden Secuencial:</strong> Las casillas del Marcador deben llenarse de izquierda a derecha, fila por fila (1 a 48).
          </p>
          <p className="text-[11px] leading-relaxed text-amber-950 dark:text-amber-200">
            • <strong>Penalización:</strong> Si el Buscador toca un número incorrecto, pierde 2 casillas de su propia puntuación.
          </p>
          <p className="text-[11px] leading-relaxed text-amber-950 dark:text-amber-200">
            • <strong>Resolución de Latencia:</strong> El servidor sella Time_Stop al instante del STOP. Cualquier clic posterior queda anulado.
          </p>
          <p className="text-[11px] leading-relaxed text-amber-950 dark:text-amber-200">
            • <strong>Fin de Partida:</strong> Quien llene sus 48 casillas gana, o bien quien tenga más al agotarse los 35 números.
          </p>
        </div>

        <button onClick={onClose}
          className="w-full py-3 rounded-xl bg-gradient-to-r from-zinc-900 to-zinc-800 dark:from-zinc-100 dark:to-zinc-200 text-white dark:text-zinc-900 font-display font-black text-xs hover:opacity-95 transition-opacity cursor-pointer shadow-md">
          ¡Entendido!
        </button>
      </div>
    </div>
